  ...
  function lookup(pid, options, callback) {
    ...
    function calculateUsage(err, stat) {
      if(err) {
        ...
        usageData['cpu'] = cpuUsage.cpu;
        usageData['cpu_user'] = cpuUsage.cpu_user;
        usageData['cpu_system'] = cpuUsage.cpu_system;
...
function calculateCpuUsage(sysinfo, uptime, stat) {
  ...
  return {
    cpu: (totalTime / processUptime) * 100,
    cpu_user: (userTime / processUptime) * 100,
    cpu_system: (systemTime / processUptime) * 100
  };
}
...
function calculateCpuUsageFromHistory(sysinfo, uptime, stat, lastUsage) {
  ...
  var userTimeSinceLast = userTime - lastUserTime;
  var systemTimeSinceLast = systemTime - lastSystemTime;
  var timeSpent = uptime - lastUsage.uptime;

  return {
    cpu: (usedTimeSinceLast / timeSpent) * 100,
    cpu_user: (userTimeSinceLast / timeSpent) * 100,
    cpu_system: (systemTimeSinceLast / timeSpent) * 100
  };
}
